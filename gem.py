...
updater = telegram.ext.Updater(token=CONFIG['802645092:AAF59zH8ZZnrijo8v8ktevc1CeL7qaULcs4'])
...
def process_msg(bot, update):
    ...
    if update.message.from_user.id == update.message.chat_id:
        markup = \
            telegram.InlineKeyboardMarkup([[telegram.InlineKeyboardButton("Хз чо такое"
                , callback_data='submission_type:real'),
                telegram.InlineKeyboardButton("Нет",
        ...
        if update.message.forward_from != None \
            ...
            if update.message.forward_from_chat != None:
                markup = \
                    telegram.InlineKeyboardMarkup([[telegram.InlineKeyboardButton("Хз чо такое 2"
                        , callback_data='submission_type:real')],
                        [telegram.InlineKeyboardButton("Отмена",
                        callback_data='cancel:submission')]])
                ...
                markup = \
                    telegram.InlineKeyboardMarkup([[telegram.InlineKeyboardButton("Хз чо такое 3"
                        , callback_data='submission_type:real')],
                        [telegram.InlineKeyboardButton("Отмена",
                        callback_data='cancel:submission')]])
...
def anonymous_post(bot, msg, editor):
    ...
    submission_list[str(CONFIG['Group_ID']) + ':'
                    + str(msg.message_id)]['posted'] = True
    bot.edit_message_text(text="Новый пост\nПубликации: ["
                          + submission_list[str(CONFIG['Group_ID'])
                          ...
                          + """)
Источник: Резерв
Рецензенты: [""" + editor.name
                          + '](tg://user?id=' + str(editor.id)
                          + ")\nПринято", chat_id=CONFIG['Group_ID'],
                          parse_mode=telegram.ParseMode.MARKDOWN,
...
def process_callback(bot, update):
    ...
    if query.data == 'cancel:submission':
        bot.edit_message_text(text="Отменено",
                              chat_id=query.message.chat_id,
                              message_id=query.message.message_id)
        return
    msg = "Новый пост\nПубликации: [" + query.message.reply_to_message.from_user.name \
        + '](tg://user?id=' \
    ...
    if query.data == 'submission_type:real':
        ...
        markup = \
            telegram.InlineKeyboardMarkup([[telegram.InlineKeyboardButton("Принято"
        ...
        submission_list[str(CONFIG['Group_ID']) + ':'
                        + str(fwd_msg.message_id)]['Markup_ID'] = \
            markup_msg.message_id
    bot.edit_message_text(text="Спасибо", chat_id=query.message.chat_id,
                          message_id=query.message.message_id)
    threading.Thread(target=save_data).start()
